...
namespace ArtificerMod.Content.Projectiles.AbilityAccH
{
	public class LifelineHook : ModProjectile
	{
		...
		public override void AI()
		{
			...
			else if (vectorToOwner.Length() > (IsStickingToTarget ? 1250f : 750f)) // Retract the hook if it's too far away
			{
				HookRetracting = true;
			}
		...
		public void Lifesteal(NPC target)
		{
			if (Projectile.owner == Main.myPlayer)
			{
				...
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, Projectile.velocity * 0f, ModContent.ProjectileType<LifelineHeal>(), 0, 0f, Projectile.owner, healTargetID, 3f);
			}
		}
